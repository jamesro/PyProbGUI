...
class normal(distribution):
    ...
    def __init__(self): 
        distribution.__init__(self, name = "Normal",
                        plot_title = "Normal Distribution",
                        demo_params = {"sigma" : [0.5, 1, 2, 5]},
                        parameters = {"mu" : parameter(0, -4, 4, continuous=True),
                                      "sigma" : parameter(1, 0.001, 8, continuous=True)},
                        wiki_text = "In probability theory, the normal (or Gaussian) distribution is a very common continuous probability distribution. Normal distributions are important in statistics and are often used in the natural and social sciences to represent real-valued random variables whose distributions are not known.\n\nThe normal distribution is useful because of the central limit theorem. In its most general form, under some conditions (which include finite variance), it states that averages of random variables independently drawn from independent distributions converge in distribution to the normal, that is, become normally distributed when the number of random variables is sufficiently large.",
                        x_range = np.linspace(-6,6,1000),
                        continuous = True)

    def get_distribution(self):
        return norm(self.parameters['mu'].value,self.parameters['sigma'].value)