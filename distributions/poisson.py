...
class poisson_d(distribution):
    
    def __init__(self): 
        distribution.__init__(self, name = "Poisson",
                        plot_title = "Poisson Distribution",
                        demo_params = {"lambda" : [1, 2, 5]},
                        parameters = {"lambda" : parameter(1, 0.1, 10, continuous=True)},
                        wiki_text = "In probability theory and statistics, the Poisson distribution, named after French mathematician Siméon Denis Poisson, is a discrete probability distribution that expresses the probability of a given number of events occurring in a fixed interval of time and/or space if these events occur with a known average rate and independently of the time since the last event. The Poisson distribution can also be used for the number of events in other specified intervals such as distance, area or volume.",
                        x_range = np.arange(0,20),
                        continuous = False)

    def get_distribution(self):
        return poisson(self.parameters['lambda'].value)